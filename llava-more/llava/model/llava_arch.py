...
class LlavaMetaForCausalLM(ABC):
    ...
    def encode_images(self, images,labels,input_ids):
        input_embeds = None
        if labels is None and input_ids is not None: 
            filtered_input_ids_batch = [
                     input_seq[(input_seq != -100) & (input_seq != -200)]
                     for input_seq in input_ids
            ]

            # Determine the maximum sequence length in the filtered list
            max_len = max(seq.size(0) for seq in filtered_input_ids_batch)

            pad_token_id = 128004

            padded_input_ids_batch = torch.stack([
                torch.nn.functional.pad(seq, (0, max_len - seq.size(0)), value=pad_token_id)
                for seq in filtered_input_ids_batch
             ])
            input_embeds =  self.get_model().embed_tokens(padded_input_ids_batch)
        if labels is not None and input_ids is not None:
            # Filter each input_ids sequence based on positions where the corresponding label equals -100
            filtered_input_ids_batch = [ 
                input_seq[(label_seq == -100) & (input_seq != -200)]
            ...
            pad_token_id = 128004  # Using pad token id from llama31

            # Pad each filtered sequence to the maximum length, then stack into a 2D tensor
            padded_input_ids_batch = torch.stack([
                torch.nn.functional.pad(seq, (0, max_len - seq.size(0)), value=pad_token_id)
                for seq in filtered_input_ids_batch
             ])
        ...
        #    filtered_labels_batch = [seq[seq != -100] for seq in labels]
        #    max_len = max(seq.size(0) for seq in filtered_labels_batch)
        #    pad_token_id = 128004 #llama31
        #    padded_labels_batch = torch.stack([
        #        torch.nn.functional.pad(seq, (0, max_len - seq.size(0)), value=pad_token_id)
        #        for seq in filtered_labels_batch
        #    ])

        #    labels = self.get_model().embed_tokens(padded_labels_batch)
    ...
    def prepare_inputs_labels_for_multimodal(
        ...
        self.input_embeds_xattn = None        
        _labels_ = copy.deepcopy(labels)
        ...
        if type(images) is list or images.ndim == 5:
            ...
            image_features = self.encode_images(images,_labels_,input_ids)
