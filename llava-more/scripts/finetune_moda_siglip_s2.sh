#!/bin/bash
# Stage-2 finetuning: LLaVA-MoRE (LLaMA-3.1-8B, SigLIP + S2) + MoDA.
# Paper configuration. Set the env vars below to your local data paths.
...
PRETRAIN_MM_ADAPTER=${PRETRAIN_MM_ADAPTER:-./checkpoints/llava_more-siglip-s2-pretrain/mm_projector.bin}
OUTPUT_DIR=${OUTPUT_DIR:-./checkpoints/llava_more-siglip-s2-moda}
...
deepspeed --master_port $MASTER_PORT llava/train/train_mem.py \
  ...
  --report_to none \
  --run_name "llava_more_siglip_s2_moda"
